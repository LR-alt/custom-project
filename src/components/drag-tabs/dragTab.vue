	...
	<div class="write-task">
		...
		>
			<el-tab-pane v-for="item in editTabs" :name="item.name" :key="item.name">
				<!-- 动态tab页签 -->
				<span slot="label" class="wrapper">
					<!-- 文本 -->
					<!-- <div class="wrap" :class="{ active: item.isEdit} ">
						<el-input
							v-show="item.isEdit"
							v-focus
							class="edit-label"
						...
						<span
							class="text"
							:style="{ visibility: !item.isEdit ? 'visible' : 'hidden' }"
						>
							{{ item.title }}
						</span>
					</div> -->
					<editTag v-model="item.title" :isEdit="item.isEdit" @blurTag="item.isEdit = false" />
					<!-- 下拉菜单 -->
					<el-dropdown
						size="small"
		...
		>
			...
			<span slot="footer" class="dialog-footer">
				...
				<el-button size="mini" type="primary" @click="copyTask"
					>确 定</el-button
				>
			</span>
		</el-dialog>
		...
		methods: {
			...
			handleCommand(command, item) {
				if (command === 'rename') {
					...
					this.subTaskForm.taskName =
						this.subTaskOps[0] && this.subTaskOps[0].value;
